...
@implementation UXScrollView
	...
	-(void) dealloc {
		...
		UX_SAFE_RELEASE(_animationStartTime);
		UX_SAFE_RELEASE(_pages);
		UX_SAFE_RELEASE(_pageQueue);
		[super dealloc];
	}
	...
	-(BOOL) flipped {
		return _orientation == UIInterfaceOrientationLandscapeLeft || _orientation == UIInterfaceOrientationPortraitUpsideDown;
	}
	...
	-(void) layoutPage {
		...
		if (page) {
			CGAffineTransform rotation	= UXRotateTransformForOrientation(_orientation);
			...
			if (self.zoomed) {
				...
				page.frame		= CGRectMake(offset.x + frame.origin.x * zoom, 
											 offset.y + frame.origin.y * zoom,
											 frame.size.width * zoom, 
											 frame.size.height * zoom);
			}
			else {
				page.transform	= rotation;
				page.frame		= CGRectMake(offset.x + frame.origin.x,
	...
	-(UIEdgeInsets) constrainEdges:(UIEdgeInsets)edges toWidth:(CGFloat)constrainedWidth {
		...
		return UIEdgeInsetsMake(edges.top - yd / 2, edges.left - xd / 2, edges.bottom + yd / 2, edges.right + xd / 2);
	}
	...
	-(BOOL) canZoom {
		return _zoomEnabled && !_holding && (_zooming || ![_delegate respondsToSelector:@selector(scrollViewShouldZoom:)] || [_delegate scrollViewShouldZoom:self]);
	}
	...
	-(void) startTapTimer:(UITouch *)aTouch {
		_tapTimer = [NSTimer scheduledTimerWithTimeInterval:0.2 target:self selector:@selector(tapTimer:) userInfo:aTouch repeats:NO];
	}

	-(void) tapTimer:(NSTimer *)aTimer {
		_tapTimer = nil;
		if ([_delegate respondsToSelector:@selector(scrollView:tapped:)]) {
			UITouch *touch = aTimer.userInfo;
	...
	-(void) startAnimationTo:(UIEdgeInsets)edges duration:(NSTimeInterval)duration {
		if (!_animationTimer) {
			...
			UX_INVALIDATE_TIMER(_tapTimer);
			
			_animateEdges		= edges;
			_animationDuration	= duration;
			_animationStartTime = [[NSDate date] retain];
			_animationTimer		= [NSTimer scheduledTimerWithTimeInterval:0.005 
																   target:self
	...
	-(void) stopAnimation:(BOOL)resetEdges {
		if (_animationTimer) {
			...
			if ((realIndex != _centerPageIndex) || self.pinched) {
				[self moveToPageAtIndex:realIndex 
				...
				if (_touchCount == 1) {
					...
					if (_holdsAfterTouchingForInterval) {
						_holdingTimer = [NSTimer scheduledTimerWithTimeInterval:_holdsAfterTouchingForInterval
																		 target:self 
																		 selector:@selector(holdingTimer:)
																	   userInfo:nil 
																	   repeats:NO];
					}
	...
	-(void) touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
		...
		UX_INVALIDATE_TIMER(_holdingTimer);
		...
		if (_scrollEnabled && !_holding && _touchCount && !_animationTimer) {
			...
			if (((_touchCount == 2) || self.zoomed) && _zoomEnabled && !_holding) {
				// I am sure this "r" had a purpose at one point, but months after writing it I'll
	...
	-(void) deviceOrientationDidChange:(void *)object {
		/*
		UIInterfaceOrientation orientation = UXDeviceOrientation();
	...
	-(void) setDataSource:(id <UXScrollViewDataSource>)dataSource {
